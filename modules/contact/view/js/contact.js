function paint(dataString) {
    $("#resultMessage").html(dataString).fadeIn("slow");
                    
    setTimeout(function() {
        $("#resultMessage").fadeOut("slow")
    }, 5000);
                            
    //reset the form
    $('#contact_form')[0].reset();
                            
    // hide ajax loader icon
    $('.ajaxLoader').fadeOut("fast");
                            
    // Enable button after processing
    $('#submitBtn').attr('disabled', false);
                    
    /*if (dataString == "<div class='alert alert-success'>Your message has been sent </div>"){
        alert(dataString);
    }else{
        alert(dataString);
    }*/
}

$(document).ready(function(){
    // disable submit button in case of disabled javascript browsers
    $(function(){
        $('#submitBtn').attr('disabled', false);
    });
            
	$("#contact_form").validate({
				rules:{
					inputName:{
                        required: true
					},
					inputEmail:{
                        required: true,
                        email: true
					},
                    inputMessage:{
                        required: true
                    }
				},
        highlight: function(element) {
            $(element).closest('.control-group').removeClass('success').addClass('error2');},
        success: function(element) {
            $(element).closest('.control-group').removeClass('error2').addClass('success');
            $(element).closest('.control-group').find('label').remove();
        },
        errorClass: "help-inline2"
	});
	
    $("#contact_form").submit(function(){
        if ($("#contact_form").valid()){
            
            // Disable button while processing
            $('#submitBtn').attr('disabled', true);

            // show ajax loader icon
            $('.ajaxLoader').fadeIn("fast");

            var dataString = $("#contact_form").serialize();
          //  console.log(dataString);

            $.ajax({
                type: "post",
                cache:'true',
                url: '../contact/process_contact/', 
                //'index.php?module=contact&function=process_contact',
                data: dataString,
                success: function(dataString) {
                  //  console.log(dataString);
                    paint(this.dataString);
                }
            })
/*
            var request = new XMLHttpRequest();
            request.open('POST', '../contact/process_contact/', true);
            request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
            request.send(dataString);

            paint(dataString);
*/
            .fail(function() {
                console.log('fail js');
                paint("<div class='alert alert-error'>Server error. Try later...</div>");
            });
        }
        return false;
    });
});
